...
public class SmellRefactoredManager {
	...
	private String resultRefactoring;
	...
	public SmellRefactoredManager(String urlRepository, String localFolder, String initialCommit, String finalCommit,
			List<LimiarTecnica> listaLimiarTecnica, String resultEvaluation, String resultSmellRefactored,
			String resultSmellRefactoredMessage, String resultSmellRefactoredCommmit, String resultRefactoring) {
		this.urlRepository = urlRepository;
		this.localFolder = localFolder;
		this.initialCommit = initialCommit;
		this.finalCommit = finalCommit;
		this.listaLimiarTecnica = listaLimiarTecnica;
		this.resultRefactoring = resultRefactoring;
		...
		pmResultEvaluation = new CSVFile(resultEvaluation, false);
		pmResultSmellRefactored = new CSVFile(resultSmellRefactored, false);
		pmResultSmellRefactoredMessage = new CSVFile(resultSmellRefactoredMessage, false);
		pmResultSmellRefactoredCommit = new CSVFile(resultSmellRefactoredCommmit, false);
	}
	...
	public void getSmellRefactoredBetweenCommit() {
		...
		try {
			...
			File fileRefactoring = new File(resultRefactoring);
			if (!fileRefactoring.exists()) {
				final PersistenceMechanism pmRefactoring = new CSVFile(resultRefactoring, false);
			...
			try {
				CSVReader reader = new CSVReader(new FileReader(resultRefactoring));
				String[] nextLine;
	...
	private FilterSmellResult obterSmellsCommit(String commit, Repository repo) throws Exception {
		...
		if (!listCommitEvaluated.contains(commit)) {
			...
			pmResultSmellRefactoredCommit.write(commit, report.getNumberOfClasses(), report.getNumberOfMethods(), report.getSystemLOC());
		}
		...
		for (MethodDataSmelly methodSmelly : commitInitial.getMetodosSmell()) {
			...
			if (methodSmelly.getSmell().equals(typeSmell)) {
				...
				if (!refactoredMethodX)
					falsePositiveX++;
				if (!refactoredMethodD || !refactoredMethodR || !refactoredMethodV || !refactoredMethodX) {
