...
public class SmellRefactoredAllManagerOld {
	...
	private void evaluateSmellChangeOperation(ArrayList<CommitData> commitsWithRefactoringMergedIntoMaster,
		...
		HashSet<String> truePositiveA = new HashSet<String>();
		HashSet<String> truePositiveV = new HashSet<String>();
		HashSet<String> truePositiveX = new HashSet<String>();
		HashSet<String> truePositiveR = new HashSet<String>();
		HashSet<String> truePositiveD = new HashSet<String>();
			...
			if (hasRafactoringRelatedToSmell) {
				...
				for (MethodDataSmelly methodSmelly : methodsCommit.getMetodosSmell()) {
					if (methodSmelly.getSmell().equals(typeSmell)) {
						...
						if (methodSmelly.getListaTecnicas().contains("A") && !truePositiveA
						...
						if (methodSmelly.getListaTecnicas().contains("V") && !truePositiveV
								.contains(commit + methodSmelly.getNomeClasse() + methodSmelly.getNomeMetodo())) {
						...
						if (methodSmelly.getListaTecnicas().contains("X") && !truePositiveX
								.contains(commit + methodSmelly.getNomeClasse() + methodSmelly.getNomeMetodo())) {
						...
						if (methodSmelly.getListaTecnicas().contains("R") && !truePositiveR
								.contains(commit + methodSmelly.getNomeClasse() + methodSmelly.getNomeMetodo())) {
						...
						if (methodSmelly.getListaTecnicas().contains("D") && !truePositiveD
								.contains(commit + methodSmelly.getNomeClasse() + methodSmelly.getNomeMetodo())) {
		...
		float precisionA = (truePositiveA.size() + falsePositiveA.size()) != 0
				? (float) truePositiveA.size() / (truePositiveA.size() + falsePositiveA.size())
				: 0;
		float precisionV = (truePositiveV.size() + falsePositiveV.size()) != 0
				? (float) truePositiveV.size() / (truePositiveV.size() + falsePositiveV.size())
				: 0;
		float precisionX = (truePositiveX.size() + falsePositiveX.size()) != 0
				? (float) truePositiveX.size() / (truePositiveX.size() + falsePositiveX.size())
				: 0;
		float precisionR = (truePositiveR.size() + falsePositiveR.size()) != 0
				? (float) truePositiveR.size() / (truePositiveR.size() + falsePositiveR.size())
				: 0;
		float precisionD = (truePositiveD.size() + falsePositiveD.size()) != 0
				? (float) truePositiveD.size() / (truePositiveD.size() + falsePositiveD.size())
				: 0;

		float recallA = (truePositiveA.size() + falseNegativeA.size()) != 0
				? (float) truePositiveA.size() / (truePositiveA.size() + falseNegativeA.size())
				: 0;
		float recallV = (truePositiveV.size() + falseNegativeV.size()) != 0
				? (float) truePositiveV.size() / (truePositiveV.size() + falseNegativeV.size())
				: 0;
		float recallX = (truePositiveX.size() + falseNegativeX.size()) != 0
				? (float) truePositiveX.size() / (truePositiveX.size() + falseNegativeX.size())
				: 0;
		float recallR = (truePositiveR.size() + falseNegativeR.size()) != 0
				? (float) truePositiveR.size() / (truePositiveR.size() + falseNegativeR.size())
				: 0;
		float recallD = (truePositiveD.size() + falseNegativeD.size()) != 0
				? (float) truePositiveD.size() / (truePositiveD.size() + falseNegativeD.size())
				: 0;
		...
		float accuracyA = (falsePositiveA.size() + truePositiveA.size() + falseNegativeA.size()
				+ trueNegativeA.size()) != 0 ? (float) (truePositiveA.size() + trueNegativeA.size())
						/ (falsePositiveA.size() + truePositiveA.size() + falseNegativeA.size() + trueNegativeA.size())
						: 0;
		float accuracyV = (falsePositiveV.size() + truePositiveV.size() + falseNegativeV.size()
				+ trueNegativeV.size()) != 0 ? (float) (truePositiveV.size() + trueNegativeV.size())
						/ (falsePositiveV.size() + truePositiveV.size() + falseNegativeV.size() + trueNegativeV.size())
						: 0;
		float accuracyX = (falsePositiveX.size() + truePositiveX.size() + falseNegativeX.size()
				+ trueNegativeX.size()) != 0 ? (float) (truePositiveX.size() + trueNegativeX.size())
						/ (falsePositiveX.size() + truePositiveX.size() + falseNegativeX.size() + trueNegativeX.size())
						: 0;
		float accuracyR = (falsePositiveR.size() + truePositiveR.size() + falseNegativeR.size()
				+ trueNegativeR.size()) != 0 ? (float) (truePositiveR.size() + trueNegativeR.size())
						/ (falsePositiveR.size() + truePositiveR.size() + falseNegativeR.size() + trueNegativeR.size())
						: 0;
		float accuracyD = (falsePositiveD.size() + truePositiveD.size() + falseNegativeD.size()
				+ trueNegativeD.size()) != 0 ? (float) (truePositiveD.size() + trueNegativeD.size())
						/ (falsePositiveD.size() + truePositiveD.size() + falseNegativeD.size() + trueNegativeD.size())
						: 0;
		...
		pmResultEvaluation.write("True Positive  (A) = ", truePositiveA.size());
		pmResultEvaluation.write("True Positive  (V) = ", truePositiveV.size());
		pmResultEvaluation.write("True Positive (X) = ", truePositiveX.size());
		pmResultEvaluation.write("True Positive (R) = ", truePositiveR.size());
		pmResultEvaluation.write("True Positive (D) = ", truePositiveD.size());
